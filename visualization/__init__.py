"""
Visualization data provider. Computes overlay coordinates without modifying scores.
Maps lowercase column names and handles timezone properly.
"""
...
def get_blocking_levels(df: pd.DataFrame) -> Tuple[List[float], List[float]]:
    """Return top 2 swing high and low levels (prices only) for blocking lines."""
    df = _normalize_columns(df)
    if df is None or len(df) < 5:
        return [], []
...
def get_visualization_data(
    ...
    Does NOT modify scores. Only computes visualization data.
    
    Returns:
        {
            "4h": {"swing_highs": [...], "swing_lows": [...], "blocking_highs": [...], ...},
            "1h": {...},
            "15m": {...},
        }
    """
