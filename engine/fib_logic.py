...
def _range_high_low(df: pd.DataFrame, up_to_idx: int) -> Optional[Tuple[float, float]]:
    """(swing_high, swing_low) of the last completed swing structure before up_to_idx."""
    sh = get_last_confirmed_swing_high(df, up_to_idx)
    sl = get_last_confirmed_swing_low(df, up_to_idx)
...
def default_boundary_price(
    ...
    """
    levels = retracement_levels(df, i, direction)
    if levels is None:
        return None
...
def zone_dominant_boundary_price(
    ...
    """
    # Check recent bars for any Level 2 zone and use its start as boundary
